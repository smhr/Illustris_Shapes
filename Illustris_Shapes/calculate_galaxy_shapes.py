...
def particle_selection(gal_id, ptcl_coords, galaxy_table, basePath, snapNum, radial_mask=True):
    """
    Select star particles within the effective radius (2*Rh_*)
...
#smhr_start
def gas_selection(gal_id, gas_coords, galaxy_table, basePath, snapNum, radial_mask=True):
    """
    Select gas cells within the effective radius (2*Rh_*)
    ...
    # get the stellar half mass radius
    gal_rhalfs = galaxy_table['SubhaloHalfmassRadType'][:,4]/1000.0
    gal_rhalf = gal_rhalfs[gal_id]

    # use only gas cells within 2 * stellar R_half
    r = np.sqrt(np.sum(gas_coords**2, axis=1))/gal_rhalf
    if radial_mask:
        radial_mask = (r<=2.0)
    else:
        radial_mask = np.array([True]*len(gas_coords[:,0]))

    return (radial_mask)
    
def gas_selection_radtot(gal_id, gas_coords, galaxy_table, basePath, snapNum, radial_mask=True):
    """
    Select gas cells within the 2*Rh_{tot}.
    ...
    # get the stellar half mass radius
    gal_rhalfs = galaxy_table['SubhaloHalfmassRad']/1000.0
    gal_rhalf = gal_rhalfs[gal_id]
...
#smhr_end
    ...
    # save measurements
    fpath = '/work8/rad/TNG/TNG50-1/data/shape_catalogs/'
